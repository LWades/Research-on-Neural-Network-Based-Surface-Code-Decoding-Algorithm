...
L = args.dist
code = ToricCode(L)
out_dimZ = 2 * L ** 2 * args.Zstab  # 输出层维度，应该是每个位置最后出错/没错的概率？
out_dimX = 2 * L ** 2 * args.Xstab  # 输出层维度
print("out_dimZ: ", out_dimZ)
print("out_dimX: ", out_dimX)
in_dim = L ** 2 * (args.Xstab + args.Zstab)  # 输入层维度，是稳定子的个数
...
if args.eval:  # 评价model的性能；测在这个错误率下的逻辑错误率
    ...
    for i, (stab, flips) in tqdm.tqdm(enumerate(stabflipgen), total=size - 1):
        ...
        print("right correct count: ", c, " detail -> {cz: ", cz, "}, {cx: ", cx, "}")
        # if both: # 12130751
        if both and i < size:
